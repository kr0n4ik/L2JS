class ItemList {
	...
	write(packet){
		let items = this.client.inventory.get();
		packet.writeC(0x11);
		...
		for (let item of items) {
			...
			packet.writeQ(1); // Quantity
			packet.writeC(0x05);//item.getItem().getType2()); // Item Type 2 : 00-weapon, 01-shield/armor, 02-ring/earring/necklace, 03-questitem, 04-adena, 05-item
			packet.writeC(0x00); // Filler (always 0)
			packet.writeH(0); // Equipped : 00-No, 01-yes
			packet.writeQ(1); // Slot : 0006-lr.ear, 0008-neck, 0030-lr.finger, 0040-head, 0100-l.hand, 0200-gloves, 0400-chest, 0800-pants, 1000-feet, 4000-r.hand, 8000-r.hand
